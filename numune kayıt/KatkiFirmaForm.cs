...
namespace numune_kayıt
{
    public partial class KatkiFirmaForm : Form
    {
        ...
        private void DataGridViewYapılandırması()
        {
            ...
            if (dgvFirmalar.Columns["katkiFirmaId"] != null) dgvFirmalar.Columns["katkiFirmaId"].Visible = false;
        ...
        private void btnEkle_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "INSERT INTO KatkiFirmalari (firmaAdi) VALUES (@firmaAdi) ";
        ...
        private void KatkiFirmaForm_Load(object sender, EventArgs e)
        {
            FirmalariYukle();
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "UPDATE KatkiFirmalari SET firmaAdi = @firmaAdi WHERE katkiFirmaId = @firmaID ";
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            ...
            if (onay == DialogResult.Yes)
            {
                try
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        string query = "DELETE FROM KatkiFirmalari WHERE katkiFirmaId = @firmaId";
                        using (SqlCommand command = new SqlCommand(query, connection))
                        {
                            command.Parameters.AddWithValue("@firmaId", seciliId);
                            connection.Open();
        ...
        private void dgvFirmalar_SelectionChanged_1(object sender, EventArgs e)
        {
            if (dgvFirmalar.SelectedRows.Count > 0)
            {
                DataGridViewRow secilisatir = dgvFirmalar.SelectedRows[0];

                txtFirmaId.Text = secilisatir.Cells["katkiFirmaId"].Value.ToString();
        ...
        private void KatkiFirmaForm_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void FirmalariFiltrele(string aranan)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT katkiFirmaId, firmaAdi FROM KatkiFirmalari WHERE firmaAdi LIKE @aranan ORDER BY firmaAdi";
